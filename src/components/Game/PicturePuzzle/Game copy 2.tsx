...
const Game: React.FC = () => {
  ...
  // State to store random image URLs and background colors
  const [playerImages, setPlayerImages] = useState<string[]>([]);
  const [playerBackgroundColors, setPlayerBackgroundColors] = useState<string[]>([]);

  useEffect(() => {
    // Generate random images and background colors once on component mount
  ...
  useEffect(() => {
    const handleVisibilityChange = () => {
      if (document.hidden) {
        setGameRunning(false); // Stop the game when the tab is hidden
      }
    };
  ...
  useEffect(() => {
    ...
    } else if (!gameEnded && !gameRunning) {
      clearInterval(); // Clear timer when game is not running
      handleGameEnd();
    }
  ...
  // Restart the game
  const restartGame = () => {
    setShowModal(false); // Close modal before restarting game
    ...
    setGameRunning(true); // Restart the game with the timer running
  };
  ...
  const startGame = () => {
    setGameRunning(true); // Start the game with the timer running
    dispatch(selectRandomPiece()); // Ensure a question is selected
  };

  return (
    // <main className="min-h-screen container mx-auto p-5 bg-white-to-black ">
              ...
              <div className='h-[38rem] bg-blue-400 rounded-xl backdrop-blur-sm bg-opacity-20 z-10 backdrop shadow-xl text-gray-800'>
                ...
                <div>
                  ...
                  <div className='flex justify-apart items-center py-1'>
                    <p className='w-1/3 mr-2 text-center py-3 text-black'>12</p>
                ...
                <div className='bg-[#397eb7] pb-4'>
                  ...
                  {/* <p className='text-center text-sm mt-2'>2.) Click on the correct option to reveal a picture piece</p> */}
                </div>
              </div>
        ...
        <GameScoreModal title="Your Result">
          <div className="mt-2 py-12">
            <div className="h-24 w-24 bg-indigo-800 rounded-full mx-auto flex justify-center items-center">
              <div>
                <p className="text-7xl text-center"> {score}</p>
